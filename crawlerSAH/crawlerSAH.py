...
baseurl = 'http://localhost:8085/SistemaAlquiler/arrend_listhabit.jsp'
	...
	if hotelName is not None:
		...
		if hotelUrl is not None:
			...
			while hUrl != '':
				...
				browser.get('http://localhost:8085/SistemaAlquiler/' + hUrl)
				time.sleep(3)
				...
				for opinionUrl in opinionUrlsHotel:
					...
					browser1.get('http://localhost:8085/SistemaAlquiler/' + url)
					time.sleep(7)
					opinionPage = browser1.page_source
					browser1.close()
					#opinionPage = urllib2.urlopen('http://localhost:8085/SistemaAlquiler/' + url)
