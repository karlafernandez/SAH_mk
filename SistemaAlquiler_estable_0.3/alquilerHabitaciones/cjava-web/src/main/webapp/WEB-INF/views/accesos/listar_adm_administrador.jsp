...
<sec:authorize access="hasRole('MOD_SEG_LISTA_ADMINISTRADOR')">
    ...
    <script src="<c:url value='/js/accesos/listar_adm_administrador.js'/>" type="text/javascript"></script> 

    <div class="fondoGris container-fluid"> 
        <div class="row-fluid  div-titulo">
            <h3>Administracion de Accesos - Administradores</h3>
        </div>
        <div class="row-fluid div-filtros" >  
            <c:url var="url" value="/listar_seg_administrador.html" /> 
            ...
            <c:if test="${grilla.total gt 0}">
                <div id="id-table">   
                    <table id ="listado" class="table table-bordered table-hover table-striped">
                        ...
                        <tbody>
                            <c:forEach var="admin" items="${grilla.rows}"  varStatus="status">
                                <tr id="${status.index}" >
                                    <td>${(grilla.page - 1) * grilla.pagination + status.index +1}</td>
                                    <td>${admin.nomAdmin}</td>
                                    <td class="hidden-phone">${admin.documento}</td> 
                                    <td><a class="btn btn-link"  href="asignar_acceso.html?idUsuario=${admin.idAdmin}&direccion=listar_seg_administrador" ><i class="icon-pencil"></i></a></td>  
                                </tr>
                            </c:forEach>
                        </tbody>
                    </table>
